...
exports.generateThumbnail = async (file, context) => {
  ...
  const thumbnailsBucket = storage.bucket('sp24-41200-elijah-gj-thumbnails');
  const finalBucket = storage.bucket('sp24-41200-elijah-gj-final');

  //Hint Hint Hint 
  const version = process.env.K_REVISION;
  console.log(`Runing Cloud Function version ${version}`);
  ...
  if (validFile) {
    ...
    // Upload our local version of the file to the final images 
  ...
  console.log(`Deleted the uploaded file: ${gcs.File.name}`);
